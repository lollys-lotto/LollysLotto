...
/// Uses a sentinel [Pubkey] value `[255, 255, 255, ...]` to signal
/// that we have a JSON-serialized `TransactionError`.
impl From<TransactionError> for LoggedTransactionFailure {
    fn from(error: TransactionError) -> Self {
        Self {
            program: Pubkey::from([u8::MAX; 32]),
            error: serde_json::to_string(&error).unwrap_or_else(|_| error.to_string()),
        }
    }
}
