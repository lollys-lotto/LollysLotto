//! Anchor Client 0.27.0 doesn't have a non-blocking `Program` implementation,
//! and its interface is a bit cumbersome.
...
pub trait ProgramEventLogSubscriber: Clone + Send + Sync + 'static {
    ...
    fn watch_for_events(&self) -> JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match state.subscribe_to_events().await {
    ...
    async fn subscribe_to_events<'a>(self) -> Result<ProgramEventSubscription<'a>> {
        ...
        let handle = tokio::spawn(async move {
            ...
            info!("attempting to subscribe to logs {ws_url} for {program_id_str}");
            ...
            info!(
                "websocket subscription successful, logs subscription: {}",
                ws_url
            );
            ...
            while let Some(response) = notifications.next().await {
                ...
                if signature == Signature::default() {
                    info!("skipping faulty message with transaction signature of Signature::default()");
                    continue;
                }
                let slot = response.context.slot;
                let logs = response.value.logs;
                let rpc_error_response = response.value.err;
                let execution_error = logs
                    .last()
